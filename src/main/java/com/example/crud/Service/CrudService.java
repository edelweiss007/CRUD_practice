...
public class CrudService {
    ...
    public List<CrudResponseDto> getComposers() {
        ...
        //List 객체를 스트림으로 변환 -> map으로 CrudResponseDto에서 만든 클래스 타입의 of 메서드 사용
        //-> collect의 Collectors.toList()로 다시 리스트로 변환
        List<CrudResponseDto> responseDto = all.stream().map(CrudResponseDto::of).collect(Collectors.toUnmodifiableList());
    ...
    public List<CrudResponseDto> getComposer(String composer) {
        ...
        List<CrudResponseDto> responseDto = foundByComposer.stream().map(CrudResponseDto::of).collect(Collectors.toUnmodifiableList());
    ...
    public CrudResponseDto updateComposer(String composer, CrudRequestDto requestDto) throws Exception{
        ...
        if(!(requestDto.getPassword().equals(savedPassword))) {
            throw new Exception("일치하지 않는 비밀번호 입니다.");
        }
        ...
        CrudResponseDto dto = CrudResponseDto.of(composers);
    ...
    public void deleteComposer(String composer, CrudRequestDto requestDto) throws Exception {
        ...
        if(!(requestDto.getPassword().equals(responseDto.get(0).getPassword()))) {
            throw new Exception("일치하지 않는 비밀번호입니다.");
        }

        //지울 내용을 Dto -> Entity로 변환
        Composers entity = requestDto.toEntity(requestDto);

        System.out.println(requestDto.toEntity(requestDto).getComposer());

        crudRepository.deleteById(entity.getId());
    }
}
